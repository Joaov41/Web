        ...
        ScrollView(.vertical, showsIndicators: true) {
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    Text("AI Model Source")
                        .font(.subheadline)
                        .fontWeight(.medium)
                    
                    Picker("AI Model", selection: Binding(
                        get: { UserDefaults.standard.string(forKey: "aiModelSource") ?? "local" },
                        set: { UserDefaults.standard.set($0, forKey: "aiModelSource") }
                    )) {
                        Text("Local (Apple Intelligence)").tag("local")
                        Text("Cloud (Shortcuts)").tag("cloud")
                    }
                    .pickerStyle(SegmentedPickerStyle())
                    
                    Text("Choose your preferred AI processing method. Local uses Apple Intelligence on-device, Cloud uses Shortcuts app.")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Button("Reset AI Model") {
                            UserDefaults.standard.removeObject(forKey: "aiModelSource")
                        }
                        .buttonStyle(SecondaryButtonStyle())
                        
                        Text("Reset AI model selection to default (Local)")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
